...
| 2 | **上下文管理** | tiktoken 精确计数 + 窗口感知压缩 + **reactive compact**（回合中压缩），LLM 摘要压缩，保留最近6条 | 精确 token 统计，200K/1M 窗口感知，**reactive compact**（回合中压缩）、context collapse、token budget（+500k 续跑） | 🟢 小 — 已补 tiktoken 精确计数、窗口感知、回合中压缩；仍缺 context collapse 多级摘要链与 +500k 续跑 |
...
| 7 | **子 agent** | AgentTool 单 prompt 或 `tasks` 并行 fan-out（并发上限 4），结果按输入顺序汇总，单任务失败隔离 | Task 工具套件（6个），**并行 fan-out**，agent 定义文件，**team/swarm**，coordinator 模式，SendMessage | 🟡 中 — 已补并行 fan-out；仍缺任务套件与 team/swarm 协作 |
| 8 | **工具集** | ~35-40 个（覆盖面很广） | 30+，多出 NotebookEdit、computer use、push notification、REPL | 🟢 小 — 覆盖面已对齐，缺个别 |
| 9 | **Hooks** | 9 事件（pre/post_tool_use, notif, stop, user_prompt_submit, session_start/end, subagent_start/stop）+ 通用 `emit` | **27 事件**，4 种后端（prompt/agent/HTTP/命令），structured output | 🟡 中 — 已扩到 9 事件与通用 emit；仍缺全量事件与多后端 |
...
3. **精确 token 计数 + 窗口感知** — 用 `tiktoken` 替换启发式估算；压缩阈值按模型上下文窗口动态算，而不是硬编码 100K。✅ **已实现**（2026-08-02 第二批：tiktoken + `cl100k_base` 回退 + 窗口感知派生阈值）。
...
6. **并行子 agent** — Task 工具加 `concurrent` 语义，或 AgentTool 支持同时 spawn 多个。这是「能用」到「好用」的分水岭。✅ **已实现**（2026-08-02 第二批：`tasks` 数组并行 fan-out，并发上限 4，按序汇总、失败隔离）。

### 🔧 P1 — 中价值（1-3 天）

7. **Hooks 事件扩展** — 加 `stop`、`user_prompt_submit`、`session_start/end`、`subagent_start/stop`。hook 后端抽象已存在，事件是枚举问题。✅ **已实现**（2026-08-02 第二批：EVENTS 扩到 9 + 通用 `emit(event, **kwargs)`）。

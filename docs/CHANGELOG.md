...
### meeting-notes 会议纪要能力

按 OpenSpec change `openspec/changes/meeting-notes` 实现结构化会议纪要契约
（首个走完 propose → apply 闭环的规格；delta 见该 change）：

- **`MEETING_NOTES_GUIDE`**（`ohwang/prompts.py`）：系统提示常驻产出契约——固定
  三节（会议信息/议题与结论/待办事项）、落盘 `docs/meetings/<日期>-<主题>.md`
  （日期取材料明示或暗示值，缺省用当前日期）、待办同步任务列表（todo_write，
  带 status/priority）、缺失字段标「待确认」不臆造、无待办时写明「无待办」。
- **`ohwang/services/meeting_notes.py`**：确定性文件名推导 `meeting_filename()` /
  `slugify_topic()`（CJK+字母数字保留、路径分隔符清洗、空值兜底），场景测试与
  路径契约共用同一命名规则。
- **行为变更**：`test_scenario_meeting_notes` 从「根目录任意文件名」改为断言
  新契约——`docs/meetings/<日期>-<主题>.md` 三节结构 + 待办进入任务列表。

### 测试

573 个测试全绿（本轮 +9：`test_meeting_notes.py` 7 例 + 场景测试 2 例——待确认
字段 / 无待办）。
...
按 `docs/PROJECT_REVIEW.md` 建议清单收尾四项低风险债务：

- **版本号/描述统一**（§5.1）：`pyproject.toml` `0.2.0`→`0.3.0`、描述
  "coding agent"→"office agent"；`ohwang/__init__.py` 同步 `__version__ = "0.3.0"`。
- **文档数字回填**（§5.2）：README "222 tests"、ROADMAP 头部 "88 文件/222 测试"
  更新为实测 559 测试 / 75 源码文件 / 覆盖率 91%。
- **`cli.build_agent` 装配顺序硬化**（§3.1）：消除闭包前向引用——`hooks`/
  `compactor`/`policy` 构造前移到 `_agent_factory` 定义之前；`_run_locked` +
  `scheduler._runner` 移后到 `agent = Agent(...)` 构造之后直接闭包捕获；
  `tools↔agent` 的循环依赖（BriefTool 需要 `agent.iterations`）用显式
  `agent_ref` 盒子打破，替代「按行号祈祷顺序」的晚绑定。装配不再依赖
  「后文才定义的名称」。
- **装配冒烟测试**（§4）：`test_cli.py` 新增
  `test_build_agent_assembly_smoke_with_cron_background_fire` —— 以
  `no_proactive=False` 完整装配（调度线程装配期间即启动），挂 `* * * * *`
  定时任务验证后台线程经共享 run_lock 触发 `agent.run` 不炸。

### 测试

560 个测试全绿（本轮 +1：build_agent 装配冒烟 + cron 后台触发）。
...
用真实 DeepSeek 完整跑通一天办公全链路：冒烟 → 会议纪要 → 周报 → 任务管理 →
行业研究 → 代码改进 → 记忆沉淀，7 项全部完成。期间 agent 顺手发现并修复
`flags.py` 环境变量真值大小写 bug（先写测试证实 2 FAIL 再修复），全量测试
515 → **517 全绿**，修复见 `f741a5e`。

**实测暴露的问题（本轮未全部修复，记录待办）**：

| 问题 | 表现 | 状态 |
| :--- | :--- | :--- |
| Bash 分类器频繁故障 | 单日 6 次 `deepseek-v4-flash is temporarily unavailable`，复杂命令被拒、`sleep` 等规则命令可过；等待 + 重试可恢复 | 环境问题，待排查分类器后端可用性 |
| `memory_read` 多词查询未命中 | 以 `"key1 key2"` 语义过滤查询刚写入的 fact 未命中，`scope=all` 全量读取才可见 | 检索匹配逻辑可优化 |
| 非交互后台输出无反馈 | `\| tail` 缓冲令任务看似卡死（实际在跑），周报任务 10min+ 无输出被误判 | 进度反馈机制可改进 |
| 简报日期模型写错 | `web_fetch` 来源页面日期被误当作检索时间（2026-02-11），人工校验修正为 08-02 | 模型瑕疵，人工校验兜底 |
| `web_browser` 缺 playwright 被禁用 | 仅 `web_search`/`web_fetch` 可用 | 安装 playwright + chromium 即可恢复 |
